...
async fn test_multiple_server_certificates() {
    ...
    for hostname in hostnames {
        let server_config = manager.get_server_cert(hostname).await
...
async fn test_certificate_renewal() {
    ...
    let server_config1 = manager.get_server_cert(hostname).await
        .expect("Failed to generate server cert");
    
    // Get initial certificate info
    let info1 = manager.get_server_cert_info(hostname)
        .expect("Should have certificate info");
    ...
    let info2 = manager.get_server_cert_info(hostname)
...
async fn test_certificate_caching() {
    ...
    let server_config2 = manager.get_server_cert(hostname).await
...
async fn test_certificate_info_extraction() {
    ...
    manager.get_server_cert(hostname).await
        .expect("Failed to generate server cert");
    
    // Get certificate info
    let info = manager.get_server_cert_info(hostname)
        .expect("Should have certificate info");
...
async fn test_build_server_config() {
    ...
    let server_config = manager.build_server_config(hostname).await
...
async fn test_concurrent_certificate_generation() {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            manager_ref.get_server_cert(&hostname).await
        });
        handles.push(handle);
    }
