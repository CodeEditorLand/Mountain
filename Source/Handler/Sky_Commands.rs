...
// --------------------------------------------------------------------------------------------
// This module implements Tauri commands specifically invoked by the Sky frontend
// for actions that are closely tied to the native application environment or
// window management, which Mountain provides. These are typically not general
// "effects" but direct interactions.
//
// Examples:
...
/// Arguments for the `mountain_set_zoom_level` command.
#[derive(Deserialize, Debug)]
struct SetZoomLevelArgs {
...
/// Tauri command to fetch the shell environment variables of the Mountain (Tauri main) process.
/// For security, it's advisable to filter these variables before sending them to the frontend,
/// but this MVP version returns all of them.
#[tauri::command]
pub async fn mountain_fetch_shell_env(
    ...
    // Security Note: Sending all environment variables to the frontend can be a risk.
    // Consider filtering to an allow-list of known safe/necessary variables.
    // Example:
    ...
    //     .collect();
    // debug!("[SkyCommands] Returning shell env with {} variables (filtered).", filtered_env.len());
    // Ok(filtered_env)
...
/// DTO for process memory information, mimicking Electron's `ProcessMemoryInfo`.
#[derive(Serialize, Deserialize, Debug, Clone)]
#[serde(rename_all = "camelCase")]
pub struct ProcessMemoryInfoDto {
    /// Amount of memory not shared by other processes (approximate).
    pub private_bytes: usize,
    /// Amount of memory shared between processes (hard to determine accurately, often stubbed).
    pub shared_bytes: usize,
    /// Resident Set Size: Actual physical memory used by the process.
    pub resident_set_size: usize,
}

/// Tauri command to get memory usage information for the Mountain (Tauri main) process.
/// Uses the `sysinfo` crate to gather this data. This reflects the main backend process,
/// not necessarily the detailed memory usage of the webview/renderer content.
#[tauri::command]
pub async fn mountain_get_process_memory_info(
    _app_handle: AppHandle<Wry>,
) -> Result<ProcessMemoryInfoDto, String> {
    ...
    if let Some(process) = sys.process(current_pid) {
        ...
        // Approximations:
        // private_bytes: Often estimated. `virtual_memory - resident_set_size` can be a rough heuristic
        //                but isn't strictly "private" memory. For simplicity, some use virtual_memory directly
        //                or a portion of it if resident set is already accounted for.
        // shared_bytes: Very difficult to determine accurately cross-platform without deep OS specifics.
        ...
        warn!("[SkyCommands] Could not find current process (PID: {}) info in sysinfo. Returning stubbed memory info.", current_pid);
        Ok(ProcessMemoryInfoDto {
            private_bytes: 0,
            shared_bytes: 0,
            resident_set_size: 0,
