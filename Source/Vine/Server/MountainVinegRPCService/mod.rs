...
#[tonic::async_trait]
impl MountainService for Struct {
	...
	async fn process_cocoon_request(
		...
		let DispatchResult = Track::SideCarRequest::Fn::Fn(
	...
	async fn send_cocoon_notification(&self, request:Request<GenericNotification>) -> Result<Response<Empty>, Status> {
		...
		match MethodName.as_str() {
			...
			"extensionHostMessage" => {
				...
				super::Notification::Fn::Fn(self, &Parameter).await;
			},
			...
			| "webview.reveal" => {
				...
				super::Notification::Fn::Fn(self, &MethodName, &Parameter).await;
			},
