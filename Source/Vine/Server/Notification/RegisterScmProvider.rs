...
//!   1. `ProviderRegistration::RegisterProvider` records the handle so
//!      future language-feature dispatches that look up by SCM handle
//!      (rare but possible) resolve.
...
//!   3. A direct `sky://scm/register` Tauri emit covers any renderer
//!      path that listens for the simpler legacy event shape (gitlens,
//!      future custom SCM views).
//!
...
use crate::{
	ApplicationState::DTO::ProviderRegistrationDTO::ProviderRegistrationDTO, Vine::Server::MountainVinegRPCService::MountainVinegRPCService, dev_log,
};
...
pub async fn RegisterScmProvider(Service:&MountainVinegRPCService, Parameter:&Value) {
	...
	// Stable handle: same DJB hash Cocoon's ScmNamespace.ts uses, so any
	// `update_scm_group`/`unregister_scm_provider` arriving later finds
	// the entry under the identical key.
	let Handle = ScmId.as_bytes().iter().fold(0u32, |Acc, B| Acc.wrapping_mul(31).wrapping_add(*B as u32));
	...
	Service.RunTime().Environment.ApplicationState.Extension.ProviderRegistration.RegisterProvider(Handle, RegistrationDto);
