...
impl Permission {
	...
	pub fn is_in_category(&self, category:&str) -> bool { self.category == category }

	/// Get the resource part of the permission name (before the dot)
	pub fn resource(&self) -> Option<&str> { self.name.split('.').next() }

	/// Get the action part of the permission name (after the dot)
	pub fn action(&self) -> Option<&str> { self.name.split('.').nth(1) }
}
...
mod tests {
	...
	fn test_invalid_permission_name() {
		let permission = Permission::new("invalid".to_string(), "Invalid permission".to_string(), "test".to_string());
