...
/// The current state of an IPC connection
...
impl ConnectionStatus {
	...
	pub fn is_healthy(&self) -> bool { matches!(self.state, ConnectionState::Connected | ConnectionState::Connecting) }

	/// Get the duration the connection has been in its current state
	pub fn current_state_duration(&self) -> Duration { self.state_since.elapsed() }
