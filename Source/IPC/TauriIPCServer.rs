...
use ring::{aead::{self, LessSafeKey, UnboundKey, AES_256_GCM}, hmac, rand::{SystemRandom, SecureRandom}};
use serde::{Deserialize, Serialize};
use tauri::{AppHandle, Emitter, Manager};
use std::time::Duration;
use tokio::sync::{Mutex as AsyncMutex, Semaphore, RwLock};
...
impl TauriIPCServer {
    ...
    pub async fn record_performance_metrics(&self, channel: String, duration: std::time::Duration, success: bool) {
        ...
        debug!("[TauriIPCServer] Performance recorded - Channel: {}, Duration: {:?}, Success: {}", 
    ...
    pub async fn send_compressed_batch(&self, channel: &str, messages: Vec<TauriIPCMessage>) -> Result<(), String> {
        ...
        let compressed_data = compressor.compress_messages(messages)
    ...
    pub async fn send_secure(&self, channel: &str, data: serde_json::Value) -> Result<(), String> {
        ...
        let encrypted_message = secure_channel.encrypt_message(&message)
    ...
    pub async fn handle_secure_message(&self, encrypted_data: serde_json::Value) -> Result<(), String> {
        ...
        let message = secure_channel.decrypt_message(&encrypted_message)
            ...
            loop {
                ...
                if let Some(handle) = connections.get_mut(&connection_id) {
                    ...
                    if !handle.is_healthy() {
                        debug!("Connection {} marked as unhealthy (score: {:.1})", handle.id, handle.health_score);
                    }
                } else {
...
pub async fn mountain_ipc_receive_message(
    ...
    if let Some(ipc_server) = app_handle.try_state::<TauriIPCServer>() {
        ...
        if let Err(e) = ipc_server.validate_message_permissions(&message).await {
            error!("[TauriIPCServer] Permission validation failed for channel {}: {}", message.channel, e);
            
            // Log security event
            ipc_server.log_security_event(SecurityEvent {
        ...
        ipc_server.record_performance_metrics(message.channel, duration, result.is_ok()).await;
...
impl PermissionManager {
    ...
    pub async fn initialize_defaults(&self) {
        ...
        let standard_roles = vec![
            ...
            ("developer", vec!["file.read", "file.write", "config.read", "storage.read", "storage.write"]),
            ("admin", vec!["file.read", "file.write", "config.read", "config.update", "storage.read", "storage.write", "system.external"]),
        ];
