...
//! 
//! Advanced security enhancements for IPC messages including AES-256-GCM encryption,
//! HMAC authentication, and secure key management.

#![allow(non_snake_case, non_camel_case_types)]

use std::{collections::HashMap, sync::Arc, time::{Duration, SystemTime}};
use log::{debug, error, info, trace, warn};
use ring::{aead::{self, LessSafeKey, UnboundKey, AES_256_GCM, NONCE_LEN}, hmac, rand::{SecureRandom, SystemRandom}};
...
impl SecureMessageChannel {
    ...
    pub fn new(config: SecurityConfig) -> Result<Self, String> {
        ...
        info!("[SecureMessageChannel] Created secure channel with {} encryption", 
    ...
    pub async fn encrypt_message<T: Serialize>(&self, message: &T) -> Result<EncryptedMessage, String> {
        ...
        let nonce_array: [u8; NONCE_LEN] = nonce_slice.try_into()
            .map_err(|_| "Invalid nonce length".to_string())?;
        
        let aead_nonce = aead::Nonce::assume_unique_for_key(nonce_array);
        
        current_key.key.seal_in_place_append_tag(
            aead_nonce,
            aead::Aad::empty(),
            &mut in_out,
        ...
        trace!("[SecureMessageChannel] Message encrypted (size: {} bytes)", encrypted_message.ciphertext.len());
    ...
    pub async fn decrypt_message<T: for<'de> Deserialize<'de>>(&self, encrypted: &EncryptedMessage) -> Result<T, String> {
        ...
        encryption_key.key.open_in_place(nonce, aead::Aad::empty(), &mut in_out)
        ...
        // Deserialize message
        let message: T = bincode::deserialize(&in_out)
    ...
    async fn cleanup_old_keys(&self) {
        ...
        previous_keys.retain(|_, key| {
            !key.is_expired(max_age)
        });
