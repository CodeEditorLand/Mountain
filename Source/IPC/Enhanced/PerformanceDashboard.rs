...
use std::{collections::{HashMap, VecDeque}, sync::Arc, time::{Duration, Instant, SystemTime}};
...
impl PerformanceDashboard {
    ...
    pub fn new(config: DashboardConfig) -> Self {
        ...
        info!("[PerformanceDashboard] Created dashboard with {}ms update interval", 
    ...
    pub async fn end_trace_span(&self, span_id: &str) -> Result<(), String> {
        ...
        if let Some(mut span) = traces.get_mut(span_id) {
            ...
            trace!("[PerformanceDashboard] Ended trace span: {} (duration: {}ms)", 
                   span.operation_name, span.duration_ms.unwrap_or(0));
    ...
    async fn update_statistics(&self) {
        ...
        if !processing_metrics.is_empty() {
            ...
            stats.peak_processing_time_ms = processing_metrics
                .iter()
    ...
    async fn cleanup_old_data(&self) {
        let retention_threshold = SystemTime::now()
            .duration_since(SystemTime::UNIX_EPOCH)
            .unwrap_or_default()
            .as_secs() - (self.config.metrics_retention_hours * 3600);
...
impl PerformanceDashboard {
    ...
    pub fn calculate_performance_score(
        ...
        (time_score * 0.4 + error_score * 0.4 + throughput_score * 0.2).max(0.0).min(100.0)
    }
