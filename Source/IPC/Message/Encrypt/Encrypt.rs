...
//!
//! Based on VSCode's secure IPC patterns in `vs/base/parts/ipc/electron-main/ipcMain.ts`
...
/// using AES-256-GCM along with HMAC-SHA256 for additional integrity verification.
...
impl SecureMessageChannel {
    ...
    pub fn from_keys(encryption_key_bytes: &[u8], hmac_key: &[u8]) -> Result<Self, String> {
        if encryption_key_bytes.len() != 32 {
            return Err(format!("Invalid encryption key length: {} (expected 32)", 
    ...
    ///
    /// # Returns
    /// Base64-encoded identifier derived from HMAC key (for debugging/key identification)
    ///
    /// # Security
...
,
        ...
        let invalid_nonce = EncryptedMessage {
            nonce: vec
![0u8; 11],
            ciphertext: vec
![0u8; 32]
,
        ...
        let too_short = EncryptedMessage {
            nonce: vec
![0u8; 12],
            ciphertext: vec
![0u8; 15]
...
![0u8; 32],
    ...
    fn test_key_rotation() {
        ...
        // This will fail because we generated a new key - new key cannot decrypt old messages
        // That's expected behavior - key rotation means old messages become undecryptable by design
