...
impl ConnectionHealthChecker {
    ...
    ///
    /// ## Parameters
    /// - `MaxConsecutiveFailures`: Maximum consecutive failures before marking unhealthy
    /// - `MinHealthyPings`: Minimum successful pings required to consider healthy
    ...
    pub async fn CheckConnectionHealth(&self, Handle: &mut ConnectionHandle) -> bool {
        ...
        let HealthCheckResult = timeout(
            self.HealthCheckTimeout,
            self.PerformHealthCheck(Handle),
        )
        .await;
        ...
        // Update metrics
        {
            ...
            Metrics.health_status = if IsHealthy {
...
mod tests {
    ...
    async fn test_health_checker_new_with_settings() {
        let Checker = ConnectionHealthChecker::NewWithSettings(
            10,
            5,
