//! `WindServiceHandlers::mountain_ipc_invoke`
	...
	Scheduler.Submit(
		async move {
			...
			let MatchResult = match command.as_str() {
				...
				"diagnostic:log" => {
					let Tag = ArgStringOr(&Arguments, 0, "webview");
					let Message = ArgString(&Arguments, 1);
				...
				"extensions:activate" => {
					let ExtensionId = ArgString(&Arguments, 0);
				...
				"lifecycle:advancePhase" | "lifecycle:setPhase" => {
					...
					let NewPhase = ArgU64Or(&Arguments, 0, 1) as u8;
					RunTime
						.Environment
						.ApplicationState
						.Feature
				...
				"nativeHost:setMinimumSize" => {
					let Width = ArgU64Or(&Arguments, 0, 400) as u32;
					let Height = ArgU64Or(&Arguments, 1, 300) as u32;
					if let Some(Window) = ApplicationHandle.get_webview_window("main") {
					...
					if !Entries.is_empty() {
						MutateRecentlyOpened(|List| {
							...
							for Entry in Entries {
								...
								if let Some(FolderUri) = Folder.and_then(|V| VStr(&V)) {
									MergedWorkspaces
								...
								if let Some(FileUri) = File.and_then(|V| VStr(&V)) {
									MergedFiles
				...
				"tree.reveal" | "tree:reveal" => {
					use tauri::Emitter;
					let ViewId = ArgString(&Arguments, 0);
					let Handle = ArgString(&Arguments, 1);
					let Options = ArgVal(&Arguments, 2);
