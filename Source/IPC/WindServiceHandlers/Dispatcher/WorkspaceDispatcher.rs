...
use crate::Utilities::RecentlyOpened::{
...
pub async fn dispatch_workspace(
    ...
    match command {
        ...
        "workspaces:addRecentlyOpened" => {
            ...
            if !entries.is_empty() {
                MutateRecentlyOpened(|list| {
                    let mut workspaces = list.get_mut("workspaces").and_then(|v| v.as_array_mut()).cloned().unwrap_or_default();
                    let mut files = list.get_mut("files").and_then(|v| v.as_array_mut()).cloned().unwrap_or_default();
                    
                    for entry in entries {
                        let folder = entry.get("folderUri").cloned().or_else(|| entry.get("workspace").and_then(|w| w.get("configPath").cloned()));
        ...
        "workspaces:enterWorkspace" | "workspaces:createUntitledWorkspace" | "workspaces:deleteUntitledWorkspace" => Ok(Value::Null),
