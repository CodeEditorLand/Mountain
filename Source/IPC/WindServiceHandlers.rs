...
/// Atom O1 + O3 helper: map a wire command string → Echo scheduler lane.
/// Unknown commands fall back to `Priority::Normal` so typos don't silently
/// bypass the Echo queue. Decoupled from the match body so it can be unit-
/// tested against the registry independently.
fn ResolveChannelPriority(Command:&str) -> EchoPriority {
	use std::str::FromStr;
...
async fn handle_file_exists(runtime:Arc<ApplicationRunTime>, args:Vec<Value>) -> Result<Value, String> {
	...
	// Atom O5 TODO: convert to `runtime.Run(StatFile(...))` via an ActionEffect
	// constructor on Common. Current direct call already executes inside the
	// Echo dispatch closure (O1), so it inherits Scheduler.Submit scheduling;
	// converting enables retry/timeout policies and unified telemetry.
...
async fn handle_workspaces_remove_folder(runtime:Arc<ApplicationRunTime>, args:Vec<Value>) -> Result<Value, String> {
	...
	crate::ApplicationState::State::WorkspaceState::WorkspaceDelta::UpdateWorkspaceFoldersAndNotify(Workspace, Folders);
