...
//! validating that users have necessary permissions before executing operations.
//!
...
use super::super::Role::ManageRole::{Role, Permission};
use super::super::Audit::LogEvent::{SecurityEvent, SecurityEventType};
...
impl PermissionValidator {
    ...
    fn BuildOperationMapping() -> HashMap<String, Vec<String>> {
        ...
        mapping.insert(
            "native:openExternal".to_string(),
            vec!["system.external".to_string()]
        );
        mapping.insert(
            "system:execute".to_string(),
            vec!["system.execute".to_string()]
        );
        mapping.insert(
    ...
    pub fn CreateSecurityContext(
        ...
        let ValidRoles = if Roles.is_empty() {
            vec!["user".to_string()]
        } else {
            Roles
        };
        
        let ValidIpAddress = if IpAddress.is_empty() {
            "127.0.0.1".to_string()
        } else {
            IpAddress
        };
    ...
    ///
    pub async fn ValidatePermission(
        &self,
        Operation: &str,
        ...
        match result {
            ...
            Err(_) => {
                error!("[PermissionValidator] Permission validation timed out for operation: {}", Operation);
    ...
    /// Ok(()) if permission granted, Err with message if denied
    ///
    async fn ValidatePermissionInternal(
        &self,
        Operation: &str,
        Context: &SecurityContext,
    ) -> Result<(), String> {
        ...
        let RequiredPermissions = match self.OperationPermissions.get(Operation) {
            ...
            None => {
                ...
                debug!("[PermissionValidator] No specific permissions required for operation: {}", Operation);
                return Ok(());
            }
        };
        ...
        if RequiredPermissions.is_empty() {
            ...
            debug!("[PermissionValidator] Access granted (no permissions required): {} by {}", Operation, Context.UserId);
            Ok(())
        } else {
    ...
    ///
    async fn AggregateUserPermissions(
        &self,
        Context: &SecurityContext,
    ...
    pub async fn RegisterRole(&self, Role: Role) -> Result<(), String> {
        ...
        for PermissionName in &Role.Permissions {
            if !permissions_read.contains_key(PermissionName) {
                warn!("[PermissionValidator] Permission '{}' referenced by role '{}' does not exist", PermissionName, Role.Name);
            }
        }
    ...
    pub async fn RegisterPermission(&self, Permission: Permission) -> Result<(), String> {
        ...
        debug!("[PermissionValidator] Permission registered: {}", permissions.get(&Permission.Name).unwrap().Name);
        Ok(())
    }
