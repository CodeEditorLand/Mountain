...
use http::{header, Request as HttpRequest, Response as HttpResponse};
...
/// - `tls_port`: Optional TLS port for HTTPS (defaults to port + 1000 if not specified)
...
/// Supports both HTTP and HTTPS protocols with automatic TLS certificate provisioning.
#[derive(Clone)]
...
impl ServiceRegistry {
    ...
    pub fn register_with_options(
        ...
        info!(
            ...
            name, port, tls_port.unwrap_or(port + 1000), use_tls
        );
        ...
        if use_tls {
            if let Some(cert_manager) = &self.cert_manager {
                // TODO: Pre-provision TLS certificate - disabled due to Send issues
                // Certificate will be generated on-demand when needed
    ...
    pub async fn health_check(&self, name: &str) -> Result<bool, Box<dyn std::error::Error + Send + Sync>> {
        ...
        debug!("[ServiceRegistry] Performing health check for {} at {}:{}", name, addr, health_path);
                    ...
                    Ok(_) => {
                        ...
                        match stream.read(&mut buffer).await {
                            Ok(n) => {
                                ...
                                let is_healthy = response.contains("HTTP/1.1 200")
