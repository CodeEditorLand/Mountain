...
//!
//! This module provides Tauri commands for managing TLS certificates from the
//! webview.
//!
//! ## Available Commands
//!
//! - `tls_get_ca_cert` - Returns the CA certificate PEM for trust store
//!   installation
//! - `tls_get_server_cert_info` - Returns information about a server
//!   certificate
//! - `tls_renew_certificate` - Forces renewal of a server certificate
//! - `tls_get_all_certs` - Lists all cached server certificates
//! - `tls_initialize` - Initializes the certificate manager and loads/generates
//!   CA
...
pub async fn tls_renew_certificate(app_handle:AppHandle, hostname:String) -> Result<String, String> {
	...
	// TODO: The Mutex needs to be held across await points - consider using
	// tokio::sync::Mutex For now, clone the manager or refactor to avoid this
	// issue
	{
		let mut manager = cert_manager.lock().map_err(|e| format!("Failed to acquire lock: {}", e))?;
