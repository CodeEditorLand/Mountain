//! # DNS Commands Module
//!
//! This module provides Tauri commands to expose DNS server information to the webview
//! and other components. It allows querying DNS server state, zone information,
//! forward allowlist, health status, and performing DNS resolution tests.
...
use crate::Binary::Build::Scheme::DnsPort;

// Import Mist crate for DNS functionality
use mist::dns_port; // Using lowercase library name

/// DNS server startup timestamp.
///
/// This static cell stores the timestamp when the DNS server was started.
/// It is set once when the DNS server initializes and remains constant thereafter.
...
// ## Commands
//
// - [`dns_get_server_info`] - Basic DNS server information (port, status, startup time)
...
// - [`DnsPort`] - The DNS port number (from [`Entry`](super::super::Main::Entry))
//
// ## Error Handling
//
...
fn get_dns_startup_time() -> String {
    DNS_STARTUP_TIME.get().map(|s| s.clone()).unwrap_or_else(|| "unknown".to_string())
}
...
/// * `record_type` - The DNS record type (e.g., "A", "AAAA", "NS", "SOA", "DNSKEY")
...
/// * `zone_status` - Status of the editor.land zone ("active", "inactive", "error")
/// * `forward_status` - Status of forward functionality ("active", "inactive", "error")
...
/// # Returns
///
/// `Result<DnsServerInfo, String>` with DNS server information or an error message
///
/// # Errors
///
...
pub fn dns_get_zone_info(dns_port: State<DnsPort>) -> Result<ZoneInfo, String> {
    ...
    for rtype in rrsig_types {
        records.push(ZoneRecord {
            ...
            ttl: 432000,
            data: format!("{} 13 2 432000 {} {} {} editor.land.", 
                rtype, 0, 0, 0), // Placeholder signature data
        });
    }
...
pub fn dns_get_forward_allowlist(dns_port: State<DnsPort>) -> Result<ForwardAllowList, String> {
    ...
    // Return the default forward allowlist from Mist::forward_security::default_forward_allowlist()
    // The default includes: update.editor.land. and cdn.crashlytics.com.
    let domains = vec![
        "update.editor.land.".to_string(),
        "cdn.crashlytics.com.".to_string(),
    ];
...
/// # Returns
///
/// `Result<DnsResolutionResult, String>` with resolution result or an error message
///
/// # Errors
///
/// Returns an error if:
...
///
/// Performs a quick resolution test and returns a simple success/failure result.
/// Useful for health checks and automated testing.
///
/// # Parameters
///
...
/// # Returns
///
/// `Result<bool, String>` with `true` if resolution succeeds, `false` otherwise,
/// or an error message
///
/// # Errors
///
...
mod tests {
    ...
    fn test_dns_server_info_serialization() {
        let info = DnsServerInfo {
            port: 5380,
            is_running: true,
            startup_time: "2024-01-01T00:00:00Z".to_string(),
        };
