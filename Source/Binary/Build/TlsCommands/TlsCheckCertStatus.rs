//! `TlsCheckCertStatus` Tauri command - parse a cached
...
#[tauri::command]
pub async fn Fn(app_handle:AppHandle, hostname:String) -> Result<CertificateStatus, String> {
	...
	if let Some(cert_info) = manager.GetServerCertInfo(&hostname) {
