...
use tauri::{http::request::Request, http::response::{Builder, Response}};
use tauri::http::Method;
use log::{debug, error, info, warn};
use std::collections::HashMap;

use super::ServiceRegistry::{ServiceRegistry, LocalService};
...
///
/// This must be called once during application setup before any land:// requests.
...
/// - Size limits
static mut CACHE: Option<HashMap<String, CacheEntry>> = None;
...
fn parse_land_uri(uri: &str) -> Result<(String, String), String> {
    ...
    let without_scheme = uri.strip_prefix("land://")
        .ok_or_else(|| format!("Invalid land:// URI: {}", uri))?;
    
    // Split into domain and path
    let parts: Vec<&str> = without_scheme.splitn(2, '/').collect();
    
    let domain = parts.get(0)
        .ok_or_else(|| format!("No domain in URI: {}", uri))?
        .to_string();
...
fn forward_http_request(
    ...
    let parsed_url = url.parse::<http::uri::Uri>()
    ...
    let path = parsed_url.path_and_query()
    ...
    let result = std::thread::spawn(move || {
        ...
        rt.block_on(async {
            ...
            loop {
                ...
                // Check if we've read the full response (simple check for content-length or end of headers)
                ...
                // Simple heuristic: if we have a full HTTP response with Content-Length, check if we've read everything
    ...
    }).join().map_err(|e| format!("Thread panicked: {:?}", e))?;
...
///     .register_uri_scheme_protocol("land", |_app, request| {
///         land_scheme_handler(request)
///     })
/// ```
pub fn land_scheme_handler(request: &Request<Vec<u8>>) -> Response<Vec<u8>> {
    ...
    debug!("[Scheme] Routing {} {} to local service at {}", request.method(), uri, local_url);
    ...
    match result {
        Ok((status, body, mut headers)) => {
            ...
            if status == 200 && should_cache(&path) {
                let content_type = headers.get("content-type").unwrap_or(&"application/octet-stream".to_string()).clone();
                let cache_control = headers.get("cache-control").unwrap_or(&"public, max-age=3600".to_string()).clone();
...
pub fn register_land_service(name: &str, port: u16) {
    let registry = get_service_registry()
        .expect("Service registry not initialized. Call init_service_registry first.");
...
/// ```
///
/// TODO: Fix this to properly use async scheme handler with correct Tauri 2.x API
