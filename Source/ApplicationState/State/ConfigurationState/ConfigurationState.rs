...
pub struct State {
	...
	pub WorkspaceConfiguration:Arc<Mutex<serde_json::Value>>,

	/// Global memento storage for crash recovery.
	pub MementoGlobalStorage:Arc<Mutex<HashMap<String, serde_json::Value>>>,

	/// Workspace memento storage for crash recovery.
	pub MementoWorkspaceStorage:Arc<Mutex<HashMap<String, serde_json::Value>>>,
}
...
impl Default for State {
	fn default() -> Self {
		...
		Self {
			GlobalConfiguration:Arc::new(Mutex::new(serde_json::Value::Object(serde_json::Map::new()))),

			WorkspaceConfiguration:Arc::new(Mutex::new(serde_json::Value::Object(serde_json::Map::new()))),

			MementoGlobalStorage:Arc::new(Mutex::new(HashMap::new())),

			MementoWorkspaceStorage:Arc::new(Mutex::new(HashMap::new())),
		}
	}
}
...
impl State {
	...
	pub fn GetGlobalMementoValue(&self, key:&str) -> Option<serde_json::Value> {
		self.MementoGlobalStorage.lock().get(key).cloned()
	}
	...
	pub fn GetWorkspaceMemento(&self) -> HashMap<String, serde_json::Value> {
		self.MementoWorkspaceStorage.lock().clone()
	}
	...
	pub fn GetWorkspaceMementoValue(&self, key:&str) -> Option<serde_json::Value> {
		self.MementoWorkspaceStorage.lock().get(key).cloned()
	}
