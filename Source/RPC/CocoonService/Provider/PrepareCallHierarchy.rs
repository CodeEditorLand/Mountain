...
fn JsonToItem(ItemValue:&Value) -> CallHierarchyItem {
	CallHierarchyItem {
	...
	match Outcome {
		Ok(Some(Items)) => {
			let Calls:Vec<CallHierarchyCall> = Items
				...
				.map(|I| CallHierarchyCall { from_or_to:Some(JsonToItem(I)), from_ranges:Vec::new() })
				.collect();

			dev_log!(
				"provider",
				"PrepareCallHierarchy result handle={} items={}",
				Request.provider_handle,
				Calls.len()
			);

			Ok(Response::new(ProvideCallHierarchyResponse { calls:Calls }))
		},

		Ok(None) => Ok(Response::new(ProvideCallHierarchyResponse::default())),
