...
//! for handling extension command lifecycle operations within the Mountain backend.
//!
//! ## Service Capabilities
//!
//! - **Command Registration**: Register commands with metadata validation and deduplication
//! - **Command Execution**: Execute commands with timeout handling and result serialization
//! - **Command Unregistration**: Clean removal with cascade operations for dependent extensions
//! - **Query Operations**: Advanced filtering by extension, category, and status
...
//! ## Code Style
//!
//! - **Naming**: PascalCase, single-word, action-oriented functions (e.g., `RegisterCommand`)
...
use crate::Vine::Generated::{
    Empty, Argument,
    RegisterCommandRequest, ExecuteCommandRequest, ExecuteCommandResponse, UnregisterCommandRequest,
};
...
impl LoggingGate {
    ...
    pub fn should_log(&self, level: log::Level) -> bool {
        level <= self.config.log_gate
    }

    pub fn is_trace_enabled(&self) -> bool {
        self.should_log(log::Level::Trace)
    }

    pub fn is_debug_enabled(&self) -> bool {
        self.should_log(log::Level::Debug)
    }
}
...
impl CommandMetadata {
    pub fn new(
        id: String, extension_id: String, title: String,
        category: Option<String>, when: Option<String>,
    ) -> Self {
        Self {
            id, extension_id, title, category, when,
            enabled: true,
    ...
    pub fn average_execution_time_us(&self) -> Option<u64> {
        if self.execution_count == 0 { None } 
        else { Some(self.total_execution_time_us / self.execution_count) }
    }
}
...
impl ServiceMetrics {
    ...
    pub fn record_success(&self, command_id: &str, latency_us: u64) {
        ...
        self.execution_latency_histogram.record(latency_us, &[KeyValue::new("command", command_id)]);
    }

    pub fn record_failure(&self, command_id: &str, error_type: &str) {
        self.execution_failure_counter.add(1, &[
            KeyValue::new("command", command_id),
...
impl CommandService {
    ...
    pub fn GetStatistics(&self) -> CommandStatistics {
        ...
        CommandStatistics {
            ...
            average_execution_time_us: if total_executions > 0 {
                Some(total_time_us / total_executions)
            } else { None },
        }
    }
}
