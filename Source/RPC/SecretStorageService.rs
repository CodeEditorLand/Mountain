...
impl SecretMetrics {
	...
	pub fn record_retrieve(&self, extension_id:&str, success:bool, latency_us:u64) {
		if success {
			...
			self.retrieve_failure_counter
				.add(1, &[KeyValue::new("extension", extension_id)]);
		}
		self.operation_latency_histogram
			.record(latency_us, &[KeyValue::new("operation", "retrieve")]);
	}
...
impl SecretStorageService {
	...
	pub async fn StoreSecret(&self, request:Request<StoreSecretRequest>) -> Result<Response<Empty>, Status> {
		...
		match secret_store
			.StoreSecret(extension_id.clone(), key.clone(), req.secret.clone())
			.await
		{
			Ok(_) => {
		...
		match secret_store.RetrieveSecret(extension_id.clone(), key.clone()).await {
			...
			Err(err) => {
				...
				#[cfg(feature = "Telemetry")]
				{
					...
					span.end();
					self.metrics
						.record_retrieve(&extension_id, false, start_time.elapsed().as_micros() as u64);
				}
...
mod tests {
	use super::*;
	// DEPENDENCY: Tests require full secret storage implementation including:
