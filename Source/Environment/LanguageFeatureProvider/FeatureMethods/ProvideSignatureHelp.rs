//! Provides signature help for a document position by invoking the matching
//! `SignatureHelp` provider in its owning sidecar.
...
pub(crate) async fn Fn(
	...
	let provider =
		super::super::ProviderLookup::get_matching_provider(environment, &document_uri, ProviderType::SignatureHelp)
			.await?;
