//! Provides folding ranges for a document by invoking the matching
//! `FoldingRange` provider in its owning sidecar.
...
pub(crate) async fn Fn(
	...
	let provider =
		super::super::ProviderLookup::get_matching_provider(environment, &document_uri, ProviderType::FoldingRange)
			.await?;
