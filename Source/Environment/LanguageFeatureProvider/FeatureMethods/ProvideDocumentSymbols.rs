//! Provides document symbols (outline) by invoking the matching
//! `DocumentSymbol` provider in its owning sidecar.
...
pub(crate) async fn Fn(
	...
	let provider =
		super::super::ProviderLookup::get_matching_provider(environment, &document_uri, ProviderType::DocumentSymbol)
			.await?;
