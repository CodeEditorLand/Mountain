//! Provides reference locations for a document position by invoking the
//! matching `References` provider and deserialising to `Vec<LocationDTO>`.
...
pub(crate) async fn Fn(
	...
	let provider =
		super::super::ProviderLookup::get_matching_provider(environment, &document_uri, ProviderType::References)
			.await?;
