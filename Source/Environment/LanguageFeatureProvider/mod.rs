...
//!
//! Implements `LanguageFeatureProviderRegistry` for `MountainEnvironment`,
//! delegating all operations to atomic helper modules.
...
mod ProviderLookup;
mod FeatureMethods;
mod InvocationHelpers;

#[async_trait]
impl LanguageFeatureProviderRegistry for Struct {
	async fn RegisterProvider(
		...
		Registration::register_provider(self, SideCarIdentifier, ProviderType, SelectorDTO, ExtensionIdentifierDTO, OptionsDTO).await
	}
	...
	async fn ProvideCodeActions(
		...
		FeatureMethods::provide_code_actions(self, DocumentURI, PositionDTO, ContextDTO).await
	}
	...
	async fn ProvideReferences(
		...
		FeatureMethods::provide_references(self, DocumentURI, PositionDTO, IncludeDeclaration).await
	}

	async fn PrepareRename(&self, DocumentURI:Url, PositionDTO:PositionDTO) -> Result<Option<Value>, CommonError> {
		FeatureMethods::provide_definition(self, DocumentURI, PositionDTO).await.map(|_| None)
	}
