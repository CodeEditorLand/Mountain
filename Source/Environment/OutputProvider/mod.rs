...
//!
//! Implements the [`OutputChannelManager`](CommonLibrary::Output::OutputChannelManager) trait for
//! [`MountainEnvironment`](crate::Environment::MountainEnvironment::MountainEnvironment).
//!
//! This provider manages multiple output channels (e.g., 'Extension Host', 'JavaScript', 'Git'),
//! handling channel lifecycle, content management, and UI visibility. It maintains in-memory
//! buffers with size limits and emits Tauri events to the Sky frontend for UI updates.
//!
...
//!
//! The single `impl OutputChannelManager for MountainEnvironment` block in this file delegates
//! to those helper functions. This satisfies Rust's orphan rules while keeping code organized.
