...
use crate::Handler::commands as CommandHandler;
...
impl CommandExecutor for MountainEnvironment {
	...
	async fn ExecuteCommand(&self, CommandIdentifier:String, Argument:Value) -> Result<Value, CommonError> {
		CommandHandler::ExecuteCommandLogic(&self.ApplicationHandle, CommandIdentifier, Argument).await
	}
	...
	async fn RegisterCommand(&self, SidecarIdentifier:String, CommandIdentifier:String) -> Result<(), CommonError> {
		CommandHandler::RegisterCommandLogic(&self.ApplicationHandle, SidecarIdentifier, CommandIdentifier).await
	}
	...
	async fn UnregisterCommand(&self, SidecarIdentifier:String, CommandIdentifier:String) -> Result<(), CommonError> {
		CommandHandler::UnregisterCommandLogic(&self.ApplicationHandle, SidecarIdentifier, CommandIdentifier).await
	}
	...
	async fn GetAllCommands(&self) -> Result<Vec<String>, CommonError> {
		CommandHandler::GetAllCommandsLogic(&self.ApplicationHandle).await
	}
}
