...
//!
//! This module provides comprehensive metrics collection for the Mountain
//! application. It tracks performance indicators, operational metrics, and
//! health statistics.
//!
//! ## Metrics Categories
//!
...
//! - **Business Metrics**: Command usage, extension lifecycle, user
//!   interactions
//!
//! ## Usage Example
//!
...
impl MetricsRegistry {
	...
	pub fn new(max_entries:usize) -> Self {
		Self { metrics:Arc::new(RwLock::new(Vec::with_capacity(max_entries))), max_entries }
	}
	...
	/// Get all metrics
	pub fn get_all_metrics(&self) -> Vec<Metric> { self.metrics.read().clone() }
...
/// Record a counter metric
pub fn record_counter(name:&str, value:f64) { GLOBAL_REGISTRY.record_counter(name, value, HashMap::new()); }

/// Record a gauge metric
pub fn record_gauge(name:&str, value:f64) { GLOBAL_REGISTRY.record_gauge(name, value, HashMap::new()); }

/// Get all metrics
pub fn get_all_metrics() -> Vec<Metric> { GLOBAL_REGISTRY.get_all_metrics() }
...
pub fn initialize_metrics() -> Result<(), Box<dyn std::error::Error>> { Ok(()) }
