...
pub fn initialize_tracing() -> Result<(), Box<dyn std::error::Error>> { Ok(()) }
...
pub async fn instrument_rpc<F, T, E>(service_name:&str, method_name:&str, operation:F) -> Result<T, E>
where
...
#[cfg(not(feature = "Telemetry"))]
pub async fn instrument_rpc<F, T, E>(_service_name:&str, _method_name:&str, operation:F) -> Result<T, E>
where
	F: std::future::Future<Output = Result<T, E>>, {
	operation.await
}
...
pub async fn instrument_command<F, T>(command_name:&str, operation:F) -> Result<T, Common::Error::Error>
where
...
#[cfg(not(feature = "Telemetry"))]
pub async fn instrument_command<F, T>(_command_name:&str, operation:F) -> Result<T, Common::Error::Error>
where
	F: std::future::Future<Output = Result<T, Common::Error::Error>>, {
	operation.await
}
