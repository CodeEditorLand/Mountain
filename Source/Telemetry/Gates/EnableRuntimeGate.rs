...
//!
//! TODO: the original implementation stored the set in `OnceLock<HashSet>`
...
//! `parking_lot::RwLock`) before this function can mutate. For now it is
//! a no-op shim that preserves the signature so call sites compile, and
