...
use tauri::{ApplicationHandle, Manager, Runtime, State, command};
...
use crate::{Handler, RPC, RunTime::ApplicationRunTime::ApplicationRunTime}; // Assuming a future RPC module for fallback
...
pub async fn DispatchCommand<R:Runtime>(
	...
	match EffectCreation::CreateEffectForFrontendCommand(&ApplicationHandle, &Command, Argument) {
...
pub async fn DispatchSidecarRequest<R:RunTime>(
	...
	match EffectCreation::CreateEffectForSidecarRequest(&MethodName, &Parameters) {
		...
		Err(_) => {
			...
			RPC::RouteRpcCall(ApplicationHandle, RunTime, SidecarIdentifier, MethodName, Parameters).await
		},
	}
}
