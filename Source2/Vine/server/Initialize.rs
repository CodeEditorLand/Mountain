...
///
/// This function retrieves the core `AppRuntime` from Tauri's managed state,
/// instantiates the gRPC service implementation (`MountainVineGrpcService`),
/// and uses `tonic` to serve it at the specified address.
///
/// @param AppHandle - The Tauri application handle.
/// @param AddressString - The address and port to bind the server to (e.g.,
/// "[::1]:50051").
pub fn Initialize(AppHandle:AppHandle<Wry>, AddressString:String) {
	tokio::spawn(async move {
		...
		// Retrieve the managed AppRuntime instance. This is the core execution engine.
		let Runtime = match AppHandle.try_state::<Arc<AppRuntime>>() {
			Some(rt) => rt.inner().clone(),
			None => {
				error!("[VineServer] CRITICAL: AppRuntime not found in Tauri state. Server cannot start.");
				return;
			},
		};

		// Instantiate our gRPC service implementation.
		let MountainService = MountainVineGrpcService::New(AppHandle.clone(), Runtime);
