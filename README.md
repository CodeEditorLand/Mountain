...
**Mountain** 🏔️ is the foundational `Rust`/`Tauri` backend for the `Land` Code
Editor. It implements the abstract service `trait`s defined in the `Common`
crate, providing native-speed implementations for filesystem I/O, process
...
(`Node.js`) extension host sidecar via `gRPC`, and serves as the backend for the
`Wind` layer through `Tauri` commands and events. As the bedrock of the entire
...
component in the Land monorepo. It depends on:
